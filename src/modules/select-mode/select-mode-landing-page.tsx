...
export const SelectModeComponet: React.FunctionComponent<IDefaultProps> = (
  ...
  return (
    <>
      <TextWrapper>
        <Title boldness={BoldnessLevels.BOLD}>
          There are currently two bots managed:{' '}
        </Title>
        <BotText>
          <BotName tag="span" boldness={BoldnessLevels.BOLD}>
            Dumpster Fire Bot:{' '}
          </BotName>
          <BotInfo>
            Asks a fun question in dmv-social every day @1:30 PM EST
          </BotInfo>
        </BotText>
        <BotText>
          <BotName tag="span" boldness={BoldnessLevels.BOLD}>
            Zoom Room Daily Theme KABOOM:{' '}
          </BotName>
          <BotInfo>
            Selects a theme for fins to use as their Zoom virtual background.
            Posts @ 8:00 AM EST
          </BotInfo>
        </BotText>

        <CenterTitle boldness={BoldnessLevels.SEMI_BOLD}>
          Select an image below to submit ideas!{' '}
        </CenterTitle>
      </TextWrapper>
      <ButtonWrapper>
        <ImageButton onClick={() => chooseSiteMode(ApprovalMode.QUESTIONS)}>
          {' '}
          <SizedDumpster />{' '}
        </ImageButton>
        <ImageButton
          onClick={() => chooseSiteMode(ApprovalMode.ZOOM_BACKGROUNDS)}
        >
          {' '}
          <SizedZoomLogo />
        </ImageButton>
